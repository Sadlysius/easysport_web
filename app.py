...
export FLASK_APP=processWebhook.py
...
def login():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        cursor.execute('SELECT * FROM a_usuario WHERE nombre = %s', (username,))
        ...
        if account:
            ...
            print('Pusiste:', password)
            print('ES:', password_rs)
...
def profile(): 
    ...
    # Check if user is loggedin
    if 'loggedin' in session:
        cursor.execute('SELECT * FROM a_usuario WHERE id = %s', [session['id']])
        account = cursor.fetchone()
